...
const tokenContractAddresses = JSON.parse(fs.readFileSync('./tokenContractAddresses'));

const jsonInterface = JSON.parse(fs.readFileSync('./../build/contracts/RecurringERC20.json')).abi;

// Allows for the scanning of multiple smart contracts
const contracts = [];

for (let i = 0; i < tokenContractAddresses.length; i++) {
  contracts.push(new Contract(jsonInterface, tokenContractAddresses[i]));
}
...
for (let i = 0; i < contracts.length; i++) {
  async function asyncCall() {
    ...
    // Scans smart contracts specified in the tokenContractAddresses file for the block number being greater than
